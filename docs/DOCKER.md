...
| Every task ends `failed:verify`; logs show `Could not load the "sharp" module` / `Error: Cannot find module` | The repo's **`node_modules` was installed on the host** (e.g. Windows/macOS) and mounted in — native modules are built for the wrong OS, so tests fail in the Linux container. Install deps **inside** the container instead: `docker exec hephaestus sh -c 'cd /projects/<repo> && npm ci'` (this overwrites the host `node_modules` — so use a separate node_modules volume if you also run the project on the host), or don't mount `node_modules` at all. Integration tests that need a DB/services will also fail unless you provide them. HEPHAESTUS won't gate on a command that's red on the baseline (it's marked *advisory* in `.hephaestus/memory/verify.md`), but the change's own touched tests must still pass. |
